...
// moved to dht22 lib for easy mutex access
void DHT_task(void* pvParameter) {
    if (pthread_mutex_init(&spiffsMutex, NULL) != 0) {
        ESP_LOGE(TAG, "Failed to initialize the spiffs mutex, aborting\n");
        vTaskDelete(NULL);
        return;
    }
...
float getHumidity() {
    ...
    // I added a mutex here because the value is written by a second thread
...
int readDHT() {
    ...
    // may be accessed in a concurrent way, wherefore better use mutex
    if (pthread_mutex_lock(&spiffsMutex) == 0) {
        humidity = dhtData[0];
        humidity *= 0x100;  // >> 8
        humidity += dhtData[1];
